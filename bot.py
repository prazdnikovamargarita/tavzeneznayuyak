...
def choose_storage(message):
    try:
        ...
        if credentials is None or credentials.invalid:
            ...
            bot.register_next_step_handler(msg, enter_cred_code)
...
def enter_cred_code(message):
    chat_id = message.chat.id
    global code
    code = message.text
    
    #Google_Authorizate(creds=creds, site_api=site_api, storage=storage, code=code)
    bot.send_message(message.chat.id, "Choose period", reply_markup=markup_choose_period)
...
def with_date_period(message, date):
    try:
        ...
        GA = Google_Authorizate(creds=creds, site_api=site_api, storage=stor.stor, code=code)
...
def send_messadge():
    try:
        ...
        bot.send_message("308984767", 'не сьогодні')
        bot.send_message("458888496", 'не сьогодні')

if __name__ == '__main__':

    #schedule.every(3).minutes.do(send_messadge)
    schedule.every().day.at("07:00").do(send_messadge)
